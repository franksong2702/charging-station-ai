...
def main():
    """主函数"""
    input_file = '/workspace/projects/assets/知识库v1.1.xlsx'
    output_file = '/workspace/projects/assets/知识库v1.1_带审核结果.xlsx'
    ...
    # 显示部分示例
    print("\n🔴 兜底性内容示例（前5条）:")
    fallback_examples = df[df['审核分类'].str.contains('兜底', na=False)].head(5)
    for idx, row in fallback_examples.iterrows():
        print(f"{row['审核标识']} {row['问题']}")
        print(f"   建议: {row['处理建议']}")
        print(f"   原因: {row['审核原因']}")
        print()
