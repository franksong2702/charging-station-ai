...
2. **优化 `clarify` 阶段**：用于收集详情（地点、时间等）
3. **保持 `summary_collect` 阶段**：用于收集手机号/车牌号
4. **修改后的流程**：`ask_problem` → `clarify` → `summary_collect` → `confirm` → `done`

**修改文件**:
- `src/graphs/nodes/fallback_node.py` - 新增 ask_problem 阶段，优化 clarify 阶段

**测试结果**:
| 测试场景 | 用户输入 | AI 回复 | 测试结果 |
|---------|---------|---------|---------|
| 用户情绪激动 | "什么破系统，充不上电" | 先安抚强烈情绪，然后进入 ask_problem 阶段追问问题 | ✅ 通过 |
