...
|-------|---------|---------|---------|---------|
| should_process_voice | `graph.py` | VoiceInputCheck | 判断是否有语音输入 | "语音处理"→asr, "直接处理文字"→intent_recognition |
| should_route_intent | `graph.py` | IntentRouteCheck | 意图路由 | "使用指导"/"故障处理"→knowledge_qa, "兜底流程"→fallback, "不满意"→dissatisfied, "满意"→satisfied, "评价反馈"→feedback, "退出兜底"→clear_fallback_state→knowledge_qa |
| should_create_case | `graph.py` | CaseConfirmedCheck | 工单确认判断 | "创建工单"→create_case, "继续兜底"→save_history |
